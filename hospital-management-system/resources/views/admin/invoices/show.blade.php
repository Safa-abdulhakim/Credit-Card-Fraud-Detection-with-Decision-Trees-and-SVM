...
@section('page-actions')
    <div class="d-flex gap-2">
        <a href="{{ route('admin.invoices.print', $invoice) }}" class="btn btn-outline-secondary" target="_blank"><i class="fas fa-print me-2"></i>Print</a>
        <a href="{{ route('admin.invoices.edit', $invoice) }}" class="btn btn-primary"><i class="fas fa-edit me-2"></i>Edit</a>
    </div>
@endsection
@section('content')
<div class="row g-3">
    <div class="col-12 col-lg-8">
        ...
        <div class="card mb-3">
            <div class="card-header d-flex align-items-center justify-content-between">
                <h6 class="mb-0 fw-semibold">Invoice #{{ $invoice->invoice_number }}</h6>
                @php
            ...
            <div class="card-body">
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        ...
                        <tr><td>Consultation Fee</td><td class="text-end">${{ number_format($invoice->consultation_fee, 2) }}</td></tr>
                        @endif
                        @if($invoice->medicine_fee > 0)
                        <tr><td>Medicine / Pharmacy</td><td class="text-end">${{ number_format($invoice->medicine_fee, 2) }}</td></tr>
                        @endif
                        @if($invoice->test_fee > 0)
                        <tr><td>Laboratory / Tests</td><td class="text-end">${{ number_format($invoice->test_fee, 2) }}</td></tr>
                        @endif
                        @if($invoice->other_fee > 0)
                        <tr><td>Other Charges</td><td class="text-end">${{ number_format($invoice->other_fee, 2) }}</td></tr>
                        @endif
                        @if($invoice->discount > 0)
                        <tr class="text-danger"><td>Discount</td><td class="text-end">-${{ number_format($invoice->discount, 2) }}</td></tr>
                        @endif
                    </tbody>
                    <tfoot>
        ...
        <div class="card">
            <div class="card-header d-flex align-items-center justify-content-between">
                <h6 class="mb-0 fw-semibold"><i class="fas fa-money-bill-wave me-2 text-success"></i>Payment History</h6>
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table mb-0">
                        <thead class="table-light">
                            <tr><th>Date</th><th>Amount</th><th>Method</th><th>Transaction ID</th><th>Actions</th></tr>
                        </thead>
                        <tbody>
        ...
        <div class="card sticky-top" style="top:80px;">
            <div class="card-body">
                ...
                <div class="mb-2 d-flex justify-content-between">
                    <span class="text-muted">Invoice #</span><strong>{{ $invoice->invoice_number }}</strong>
                </div>
                <div class="mb-2 d-flex justify-content-between">
                    <span class="text-muted">Total Amount</span><strong>${{ number_format($invoice->total_amount, 2) }}</strong>
                </div>
                <div class="mb-2 d-flex justify-content-between text-success">
                    <span>Amount Paid</span><strong>${{ number_format($invoice->paid_amount, 2) }}</strong>
                </div>
                @if($invoice->remaining_amount > 0)
                <div class="mb-2 d-flex justify-content-between text-danger">
                    <span>Balance Due</span><strong>${{ number_format($invoice->remaining_amount, 2) }}</strong>
                </div>
                @endif
                @if($invoice->due_date)
                <div class="mb-2 d-flex justify-content-between">
                ...
                <div class="mb-2 d-flex justify-content-between">
                    ...
                    <a href="{{ route('admin.appointments.show', $invoice->appointment) }}" class="small">View</a>
                </div>
                @endif
                <hr>
