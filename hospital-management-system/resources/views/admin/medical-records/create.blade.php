...
@section('content')
<div class="card">
    <div class="card-header"><h6 class="mb-0 fw-semibold"><i class="fas fa-file-medical me-2 text-info"></i>معلومات السجل الطبي</h6></div>
    <div class="card-body">
        <form action="{{ route('admin.medical-records.store') }}" method="POST">
            ...
            <div class="row g-3">
                ...
                <div class="col-12">
                    <label class="form-label fw-semibold">الأعراض <span class="text-danger">*</span></label>
                    <textarea name="symptoms" class="form-control @error('symptoms') is-invalid @enderror" rows="3" placeholder="صف الأعراض...">{{ old('symptoms') }}</textarea>
                ...
                <div class="col-12">
                    ...
                    <textarea name="diagnosis" class="form-control @error('diagnosis') is-invalid @enderror" rows="3" placeholder="تشخيص الطبيب..." required>{{ old('diagnosis') }}</textarea>
                    @error('diagnosis')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-12">
                    <label class="form-label fw-semibold">خطة العلاج</label>
                    <textarea name="treatment" class="form-control @error('treatment') is-invalid @enderror" rows="3" placeholder="خطة العلاج أو الإجراءات...">{{ old('treatment') }}</textarea>
                    @error('treatment')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-6">
                    <label class="form-label fw-semibold">ضغط الدم</label>
                    <input type="text" name="blood_pressure" class="form-control" value="{{ old('blood_pressure') }}" placeholder="مثال: 120/80 mmHg">
                </div>
                <div class="col-md-6">
                    <label class="form-label fw-semibold">درجة الحرارة</label>
                    <input type="text" name="temperature" class="form-control" value="{{ old('temperature') }}" placeholder="مثال: 98.6°F">
                </div>
                <div class="col-md-6">
                    <label class="form-label fw-semibold">الوزن</label>
                    <input type="text" name="weight" class="form-control" value="{{ old('weight') }}" placeholder="مثال: 70 kg">
                </div>
                <div class="col-md-6">
                    <label class="form-label fw-semibold">معدل النبض</label>
                    <input type="text" name="pulse_rate" class="form-control" value="{{ old('pulse_rate') }}" placeholder="مثال: 72 bpm">
                </div>
                <div class="col-12">
                ...
                <div class="col-12 d-flex gap-2 mt-2">
                    <button type="submit" class="btn btn-primary"><i class="fas fa-save me-2"></i>حفظ السجل</button>
                    <a href="{{ route('admin.medical-records.index') }}" class="btn btn-secondary">إلغاء</a>
                </div>
            </div>
        </form>
    </div>
